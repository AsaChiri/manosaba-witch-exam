...
Fan-made web experience for 魔法少女ノ魔女裁判 (Manosaba): the visitor is diegetically *examined for the witch factor*, receives a verdict, and gets their 魔女図鑑 card. This is the **v2 runtime** of the witch-card project; structural quiz data and the growing card corpus are compiled from `D:\Manosaba_Script_Project_Workspace`. The four committed `content/quiz/strings.<locale>.json` files are the authoritative exception and are never generated by the compiler.
...
- `src/i18n/*.json` — UI-chrome strings, one flat catalog per locale (zh-CN authoritative for tone; zh-TW derived manually). Question text comes from the four authoritative, manually maintained `content/quiz/strings.<locale>.json` files.
...
npm -C tools/compiler run compile   # compile structure, routing, cards, characters
npm -C tools/compiler run verify    # round-trip against certified scorer outputs
```
...
- Game assets : ONLY what `src/assets/game/ATTRIBUTION.md` lists ships — two room backgrounds as darkened atmosphere (landing = courtroom, record pages = the gallery wall; the same rooms inside the OG plates) and the 13 WitchBook profile portraits for the character OG and the file photo pinned to the warden's note on the special record. Everything else (seal, rose windows, frames, buttons, icons) stays SVG/CSS. Re-import with `npm run import:assets`. The character OG and the `/c/` page meta name the character and NEVER her magic (the pairing is a story spoiler); the `/c/` page itself sits behind a plain spoiler gate.
