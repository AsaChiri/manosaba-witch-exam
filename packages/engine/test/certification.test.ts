...
// FNV-1a correctness vectors on two long canonical strings: the retired v2
// (82-slot) and v1 (81-slot) shapes from certification.md §3. Different inputs,
// different pinned outputs — they exercise the hash on more than one string.
// (v1/v2/v3 are not comparable; nothing shipped under v1/v2.)
//
// Spelling note (2026-09-02, owner decision: the Performer sub-variant is `P-*`
// everywhere, never `PE-*`): certification.md §3 took its pins while the token
// read `V.CPICK:PE-2` (610 B → 0x6FC8FE2D; 599 B → 0xF6ADEE5B). With the `P-2`
// spelling used here and in the doc's printed strings, the same two strings
// are one byte shorter and hash to the values pinned below (verified against
// the reference FNV-1a parameters; the persona replay carries the reference
// cross-check).
describe("retired FNV-1a pins (certification.md §3, P-* spelling)", () => {
  const V2 =
  ...
  it("reproduces the v2 pin (609 bytes, 0xB8A39548; PE-2 spelling was 610 B, 0x6FC8FE2D)", () => {
    expect(new TextEncoder().encode(V2).length).toBe(609);
    expect(fnv1a32String(V2) >>> 0).toBe(0xb8a39548);
  });
  it("reproduces the v1 pin (598 bytes, 0xC1B0713A; PE-2 spelling was 599 B, 0xF6ADEE5B)", () => {
    expect(new TextEncoder().encode(V1).length).toBe(598);
    expect(fnv1a32String(V1) >>> 0).toBe(0xc1b0713a);
  });
});
